...
def calculate_kpi_score(df, weights=None):
    ...
    out["score_quality"] = (
        100
        - safe_num(out["compliance_issue"]) * 20
        - safe_num(out["churn_clients"]) * 2
    ).clip(0, 100)
    ...
    choices = ["A - Top Performer", "B - Good", "C - Watchlist"]
    out["performance_band"] = np.select(conditions, choices, default="D - Action Required")
    ...
    out["bonus_policy"] = np.select(
        ...
        [
            "Bonus accelerator / assign HNW leads",
            "Standard bonus / maintain target",
            "Coaching + weekly pipeline review",
        ],
...
def generate_rm_recommendation(row):
    recs = []
    if row.get("score_revenue", 0) < 50:
        recs.append("Push brokerage revenue")
    if row.get("score_margin", 0) < 50:
        recs.append("Review margin penetration")
    if row.get("score_active_clients", 0) < 50:
        recs.append("Reactivate dormant clients")
    if row.get("score_new_accounts", 0) < 50:
        recs.append("Improve new account acquisition")
    if row.get("score_quality", 100) < 70:
        recs.append("Check compliance / churn risk")
    if not recs:
        return "Maintain performance; assign higher-value clients"
    return "; ".join(recs)
